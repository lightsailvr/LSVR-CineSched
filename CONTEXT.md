...
in progress. The shot lists (#36, tickets #37–#43) are the same on every platform.
...
|---|---|
| **Scene** | One unit of the screenplay to be shot. Modelled by `Scene` in `Models.swift`. Has a scene number, slugline-style title (`INT. KITCHEN - NIGHT`), page length in **eighths**, estimated shoot time in minutes, cast (character names), a summary, and breakdown tags. Holds a **shot list** and, while it has no shots, a **storyboard frame** of its own (#36). |
...
| **Banner** | A non-scene strip on a day: company move, meal break, or free-text notice. Still a `Scene` value with `isBanner == true` and a `BannerType`. How its strip looks is one rule for the Mac's board and the phone's lists (`BannerAppearance.swift`: `bannerFillHex`, `bannerDisplayLabel`). |
...
| **Breakdown** | Per-scene tagging of extras, props, set dressing, wardrobe, makeup/hair, vehicles, special equipment, stunts, SFX, VFX, and notes. The Breakdown Browser steps through scenes in script order; `BreakdownExporter` prints one sheet per scene. Props, special equipment and SFX read as the **breakdown union** of the scene's items and its shots' (`Scene.allProps`, `allSpecialEquipment`, `allSFX`, #37); the other categories are the scene's alone. |
| **Shot** | One setup of a scene's coverage (`Shot` in `Models.swift`, #37): a description (`details`), a duration in minutes that includes its relight (15 for a new shot), equipment, props and SFX as lists, and at most one **storyboard frame**. A shot never leaves its scene and has no stored letter. |
| **Shot list** | A scene's shots in shooting order (`Scene.shots`). Every change goes through the shot edits (`ShotEdits.swift`, on `Scene` and on `ProjectData` by scene and shot id), each ending with the **estimate rule**: while a scene has shots its `estimatedTime` is written as the sum of their durations, so the time cascade, the day totals and every export follow the shot list without knowing about it; removing the last shot leaves the last sum, editable again. |
| **Shot letter** | A shot's letter is its position in the shot list: A to Z, then AA, AB, … (`Shot.letter(forIndex:)`). Its displayed number is the scene number followed by the letter, no separator: 12A, and 12AA for a shot in scene 12A (`Scene.shotNumber(at:)`). A reorder re-letters; nothing is stored. |
| **Storyboard frame** | One image on a shot, or on a scene with no shots, stored in the project file as downscaled JPEG bytes (`Shot.frame`, `Scene.frame`; base64 in the JSON, ADR 0007). The first shot added to a scene takes the scene's frame (the **frame rule**), so a scene with shots carries none of its own; a first shot that brings a frame while the scene has one is refused, so no board is lost. |
| **Frame slot** | Where a storyboard frame is shown and set in the scene editor (`StoryboardFrameSlot`, #41): on a shot's page, and in a shotless scene's frame row. The frame whole (aspect-fit, never cropped, whatever its aspect) with Add Frame (Replace once there is one), Paste and Remove. The **frame sources** are the platform's (`PlatformFrameSources`, a seam): Choose File… and a drop onto the frame everywhere; paste (the Paste button, or ⌘V with the frame focused) on the Mac, the iPhone and the iPad; Photo Library… on the iPhone, the iPad and Vision Pro; Take Photo… on the iPhone and the iPad. Every source ends in `StoryboardFrame.encode(data:)` (JPEG at quality 0.8, the long edge at most 1200 px, upright, never upscaled), and the frame is part of the editor's one Save. Once the project's frames pass 20 MB the shot page says so under the slot (the **frame caption**, "Storyboard frames: 24 MB"). |
| **Shot page** | A shot's **detail page** in the scene editor's Shots section (`ShotPage` over a `ShotDraft`, #39): the description (focused on a new shot, except in the inspector), the duration (15 minutes to start), Equipment, Props and SFX (each suggesting what the project already lists in that category, and completing the item after the last comma), and the frame slot. Its footer is Remove, Duplicate Shot, Add Another (saves it and opens a fresh shot right after it) and Done. The Shots section's rows (number, description, duration, a thumbnail) reorder by drag or Move Up / Move Down and swipe to delete; while a scene has shots its estimate is read-only with "From N shots" under it. |
| **Show Shots** | The switch that unfolds every script scene's strip to show its shots: View ▸ Show Shots on Stripboard (and the toolbar's Display menu on the Mac, the View menu on the iPad), and on the iPhone a row in the Production tab's Appearance section. A strip's **chevron** flips that one strip against it (`ShotExpansion`: Show Shots plus the exceptions; switching Show Shots clears them). Window state: Show Shots is a **window preference** and the exceptions the window's own, so two windows can look different and nothing reaches the file. The Boneyard never unfolds. |
| **Shot sub-row** | One line per shot under an unfolded strip (`ShotSubRow`, #42; `PhoneShotRow` in the phone's lists, #43): the shot number, the description, the equipment dimmed and the duration, never a start or end time and never a thumbnail, on a translucent tint of the strip's color. A double-click on the Mac (a double tap on the iPad) opens the scene editor on that shot's page, a single tap on the iPad shows the page in the inspector, and a tap on the iPhone opens the editor on it. On the Mac board a sub-row drags up or down within its own scene (the **shot drag**, one undo step); anywhere else it shows no insertion line and does nothing. The phone reorders shots only in the scene editor. A strip's menu has Add Shot…, the editor opened on a new shot's page. |
| **Shots field** | The Stripboard Fields entry that prints a scene's shot count as a chip ("1 shot", "4 shots", nothing for none; `StripboardField.shots`, #42): off by default, on the Mac strip, both phone lists, the phone's Boneyard tab and the month export. The Mac Boneyard's row shows the same count whatever the fields say. |
| **Shot List export** | The seventh PDF (`ShotListExporter`, #40), portrait US Letter: the whole project (the days in schedule order, then the Boneyard in script order) or one shoot day, each day under its number and date and each scene under its number, slugline, estimate and pages. With **Include Storyboard Frames**, three shots of equal height a page, the frame fitted whole in a box about 3.3 inches wide beside the shot's number, duration, description, equipment, props and SFX; without, a table, one line per shot. A shotless scene prints as one entry under its bare number with its summary, estimate, breakdown union and own frame. Banners, meals and events never print. The scope and the frames choice are asked first (`ShotListOptionsSheet`) and remembered app-wide; File ▸ Export Shot List… and the Share menu on the Mac and the iPad, the Shot List row of the Production tab on the iPhone. |
...
| **Range preview** | `ProductionRangePreview` (#28): what Update Calendar would do before it does it, from `previewProductionRange` (the regeneration run on a copy): the day count of the new range, how many script scenes (not banners) would return to the Boneyard, and whether everything slides (`shifts`: shift mode on and the start moved) or stays on its dates, which the confirmation's copy says. The phone confirms a change that displaces any; the Mac applies without asking. |
| **Phone move** | A change to where a scene sits, made from the iPhone (#25): reorder within a day (a strip's drag handle on the Day screen or a long-press drag in the Days list), **Send to Day** (a strip's swipe or menu → a day picker → the day's end), **Move to Next Day** / **Move to Previous Day** (a strip's menu, and Next Day its swipe too → the end of the adjacent day of the schedule, the next entry of `shootDays` whatever is on it — an empty or typed day is still "tomorrow" to the scheduler; absent at the first and last day), **Add Scenes** (the Boneyard checked onto a day in display order), **Swap with Day** (two dates exchange scenes, call sheet, day type, note and events) and **Return to Boneyard** (a strip's swipe). `PhoneMoves` (wired by `PhoneEditor`, handed to the tabs) runs each as one `edit` over the pure `ScheduleMoves` functions, with the auto-meal sync of the touched days inside it, so a move is one undo step (a shake or the three-finger swipe reverses it) and the time cascade recomputes by redraw. A strip cannot be dragged across days in the Days list: a sectioned `List` does not carry a drag across its sections on iOS 27 (learnings 2026-09-21), so Send to Day is the cross-day move. |
| **Boneyard tab** | The iPhone editor's second tab (`BoneyardTab`, #27): the Boneyard as a list of strips in the Mac's six sorts (`BoneyardSort`, the same `CineSchedBoneyardSort` preference the Mac's sidebar menu writes, so both list the same order) narrowed by the **scene filter**, with the count of scenes and pages listed. A tap opens the scene editor (the phone's one, `PhoneSceneEditor`, through `PhoneDayEdits.presentSceneEditor` with the displayed rows as its siblings: Previous and Next step them, Delete removes a Boneyard scene as the Mac's Boneyard editor does, Duplicate Scene copies it), a long press is the Mac's Boneyard menu (Edit, Duplicate, Send to Day…, Delete) with the strip preview, the trailing swipe is Delete, Send to Day or Duplicate, and a duplicate scene number wears the Mac's dashed red edge. Select puts the list in edit mode (`listSelecting`, the seam over `editMode`) for a **multi-selection** that one Send to Day places on the chosen day in the list's display order (`BoneyardSelection.ordered`, never widened from the set; the same `PhoneMoves.sendToDay` as a strip's, one undo step). "+" is the **New Scene form**. A scene to show (from Search, or one just added) arrives through `revealSceneID`: the filter clears and the list scrolls to it. Its rows print the Stripboard Fields chips (the **Shots field** among them, #43) and never unfold. |
| **Scene filter / scene search** | `SceneSearch` (pure, #27): the one rule for finding a scene by what it says, as the Boneyard tab's filter (`filter`, over the sorted Boneyard, order kept) and the Search tab's search (`results(for:in:)`, over the whole project). The query is trimmed, lowercased and split on whitespace, and every word must appear somewhere in the scene: as its number, exactly or as a prefix ("12" finds 12, 12A and 120), in its slugline, in a cast name (trimmed, case-insensitive: the **character name join**'s terms), in its summary, in its real location, or in any of its shots' descriptions, equipment, props or SFX (#37). Only script scenes match: a banner, an auto-meal or a calendar event never does. A blank query filters nothing and finds nothing. A result (`SceneSearchResult`) carries the scene and its `SceneLocation`, and the shot that found it when a word was in a shot and not in the scene's own fields (`matchedShotID`, #43); the results come scheduled first, day by day in the order they are shot, then the Boneyard's in script order. The Mac toolbar's search keeps its older rule (`toolbarMatches`: the query as one phrase in the title, summary or a cast name), and reads the shots too. |
| **Search tab** | The iPhone editor's fourth tab, in the search role (`SearchTab`, #27): the system search field over the scene search, its results in two sections, the scheduled scenes (each row naming its production day and date) and the Boneyard's, with the cast (else the summary) under each. A tap opens the scene editor (the phone's one, through `PhoneDayEdits.presentSceneEditor` with the results shown as its siblings; Delete on a scheduled scene returns it to the Boneyard, the Mac's sheets' rule); the row's trailing button, its swipe and its long-press menu show the scene where it lives: **Show in Days** switches to the Days tab scrolled to the day (`scrollToDate`, the Today control's hook), **Show in Boneyard** switches to the Boneyard tab scrolled to the scene (`revealSceneID`). No recent searches. A result found through a shot names it under the row ("in shot 12B", #43). |
| **New Scene form** | `NewSceneSheet` over `NewSceneDraft` (#27): the Mac's sidebar New Scene form (`NewSceneInputView`, untouched) as an adaptive editor on the phone: number (may be blank), slugline (required), real location with the roster's autocomplete, pages and estimate (blank allowed, malformed not; an estimate left blank is derived from the pages at fifteen minutes an eighth, the Mac's rule, else none), and the type. The type picker's default (nil) reads the time of day off the slugline as the script importers do (`FinalDraftParser.TimeOfDay`, unknown reading as day), shown live as "From slugline · NIGHT"; a picked type wins. Add Scene appends `makeScene()` to the Boneyard in one edit and the Boneyard tab scrolls to it. |
...
| **Undo gestures** | Shake to undo and the three-finger undo and redo swipes on iOS and iPadOS. UIKit asks the first responder for its `undoManager`, not SwiftUI's environment, and a SwiftUI editor with no field focused has no first responder, so the gestures reached the window's empty manager and did nothing. The pasteboard responder is the answer on both editors: it carries the document's undo manager (the one `perform` registers with) and takes first-responder status whenever nothing else holds it (a run-loop observer, because the undo alert, a menu and a dismissed sheet leave none behind and post nothing). `PhoneEditor` installs it through `undoGestures(_:)`; the three-column layout hands the manager to the responder it already has; the Mac's window answers Edit ▸ Undo itself and gets nothing. A focused field keeps the gesture for its own typing. |
...
| **Export presentation** | Where an export request goes: the Mac's save panel (`FilePanels`, unchanged), or on iPad, iPhone and Vision Pro the **preview sheet** (`PDFExportPreviewSheet`: the PDF through PDFKit, Done, Share), whose share sheet offers AirDrop, Messages, Mail, Print, Save to Files and the rest for the file under its export name. `PlatformExportPresentation` (a seam) decides for `ContentView`'s call sites (`ContentView+PDFExports.swift`); the iPhone editor's are `PhoneExports`, which always previews; the iPad reaches it from the toolbar's Share menu (Month Calendar… picks its month in the options sheet), the Stripboard day header, the day inspector and the call sheet editor. |
...
PhoneEditor.swift         The iPhone editor (#24): the tab view (Days, Boneyard, Production, Search), the Today
                          control as the tab bar accessory, the edit funnel the tabs get as a closure (plain and
                          under a caller-owned gesture), the sync monitor, the palette, the derived-state cache
                          (with the Boneyard sort, the Mac's preference), the PDF preview, the cross-tab jumps,
                          the project commands for the iPad's menu bar in a narrow window (#28), and the undo
                          gestures (undoGestures, the pasteboard responder with inert actions). Platform-free.
  ...
  PhoneStripListRows.swift         a day's strips with their shot sub-rows as the two lists show them, and the
                                   list's onMove offsets mapped onto the strips alone (pure, #43).
  PhoneDayMenus.swift              the day's ⋯ / long-press menu and the event menu, one @ViewBuilder each,
                                   shared by the Days list and the Day screen.
  BoneyardTab.swift                the Boneyard tab (#27): the sorted, filtered Boneyard as strips, the editor on
                                   tap, the Mac's menu on long press, swipe Delete / Send to Day / Duplicate,
                                   Select for a multi-selection sent to a day in display order, "+" for the New
                                   Scene form.
  SearchTab.swift                  the Search tab (#27): the search field over SceneSearch.results in scheduled and
                                   Boneyard sections; a tap opens the editor, Show in Days / Show in Boneyard jump.
  ...
  PhoneDayEdits.swift              the iPhone's day edits (#26): type, note, notice strips, Set Time, the call
                                   ...
                                   presents over the editor: PhoneSceneEditor (the phone's one scene editor, by
                                   id, Previous / Next over a day's scenes or a tab's rows, Duplicate), banner,
                                   event, Set Time, call sheet; PhoneEditorUnavailable for a gone subject.
DayEdits.swift            The pure day edits (#26): Scene.duplicated() and the ProjectData mutations behind
                          PhoneDayEdits (Clear Day Type with the production range), tested in DayEditsTests.
QuickTimeEditSheet.swift  Set Time as an adaptive form on QuickTimeDraft (#26); the Mac's Stripboard presents it too.
ShotEdits.swift           A scene's shot list (#37): the letters and numbers, the edits on Scene and by id on
                          ProjectData, each ending with the estimate rule (and adding with the frame rule), fresh
                          shot ids for a copy, the breakdown unions (pure, ShotEditsTests).
StoryboardFrame.swift     A frame's stored bytes (#38): any image decoded upright, downscaled to 1200 px on its long
                          edge, JPEG at 0.8; the decode cache views read (pure, CoreGraphics and ImageIO).
StoryboardFrameView.swift A frame shown whole, aspect-fit, decoded off the main actor. Platform-free.
StoryboardFrameSlot.swift The frame slot of a shot's page and a shotless scene (#41): the frame, Add Frame / Replace,
                          Paste, Remove, a drop; every source through StoryboardFrame.encode(data:). Platform-free.
PlatformFrameSources.swift  Seam: which frame sources a platform offers (Photos, the camera, paste) and their pickers.
StoryboardFrameTotals.swift The project's frame bytes and the 20 MB caption (pure, #41).
ShotPage.swift            The scene editor's shot page and shot row (#39). Platform-free.
ShotExpansion.swift       Which strips show their shots: Show Shots and the chevrons' exceptions (pure, #42).
ShotSubRow.swift          One shot under its strip, the Mac board's and the phone's, and its tint (#42).
ShotListExporter.swift    The Shot List PDF (#40): its sections and entries (pure) and the pages, with or without frames.
ShotListPDFOptions.swift, ShotListOptionsSheet.swift   The Shot List's scope and frames choice, remembered app-wide,
                          and the options sheet every platform shows before the export (#40).
  ProductionTab.swift              the Production tab (#28): the Mac's Production, View and File menu commands
                                   ...
                                   colors, fields), each presenting an existing editor, report or setting;
                                   ProductionCommand.
    ProductionTab+Range.swift        the Project section: title, the range pickers (PhoneEditor's state),
                                     Shift Schedule, Update Calendar and its confirmation.
    ProductionTab+Exports.swift      the Export section: the seven documents through PhoneExports, the Shot List
                                     after its options sheet.
    ProductionTab+Import.swift       the Import section: Import Script… with the picker and the summary.
  PhoneExports.swift               the iPhone editor's one PDF export call site (the phone's
                                   ContentView+PDFExports): each document into the preview, a failure into the alert.
ProductionEdits.swift     The production-wide edits (character rename, lock, unlock) as pure ProjectData
                          mutations the phone and ContentView call (ProductionEditsTests).
BannerAppearance.swift    A banner strip's fill and label, the Mac row's rules as pure Scene properties both
                          the Stripboard and the phone draw (BannerAppearanceTests).
BreakdownBrowser.swift    The Breakdown Browser's script-ordered scene ids and stepping (pure, #28; the Mac's
                          browser takes its snapshot through it since #35).
SceneSearch.swift         The scene filter and search (every word somewhere; number, slugline, cast, summary, real
                          location, the shots' text; script scenes only), the result with its location and the shot
                          that matched, the Mac toolbar's phrase rule, and the Boneyard selection's display order
                          (pure, #27, #37, #43).
  PhoneMoves.swift                 the iPhone's schedule moves (#25): reorder, Send to Day, Move to Next /
                                   Previous Day, Add Scenes, Swap with Day, Return to Boneyard, each one edit;
                                   and the pickers a move presents.
  AddScenesSheet.swift             Add Scenes on the Day screen (#25): the Boneyard as a checklist, added in display order.
DaySummaries.swift        DaySummary (with the day's label), WeekStrip, todayTarget and dayScrollTarget (pure, #24).
...
ContentView.swift         The editor for one document: sidebar, toolbar, calendar and stripboard, in the Mac's
  ...
  ContentView+PDFExports.swift     every PDF export action of ContentView: build the PDFExportRequest, deliver it to
                                   the save panel (Mac, FilePanels) or the preview sheet (iOS, visionOS). The
                                   iPhone editor's are PhoneExports.swift.
...
EditorSelection.swift     The inspector's selection (a scene or a day by id), locating it in the project, pruning,
                          the by-id write-back (replaceScene, removeScene), knownLocations and the shot page's
                          breakdown suggestions (pure).
...
EditorDrafts.swift        The editors' drafts (SceneDraft with its shot list, ShotDraft, BannerDraft, CalendarEventDraft,
                          NewSceneDraft, QuickTimeDraft), pure.
...
Models.swift              All value types (Scene, Shot, ShootDay, ProjectData, CallSheetData, ...). Hand-written Codable.
ProjectCodec.swift        The one project encoder/decoder (pretty JSON with sorted keys, ISO dates, legacy shapes;
                          frames as base64, ADR 0007).
...
PDFCanvas.swift           Shared PDF drawing helper: pages, rects, lines, TextKit-compatible text on CoreGraphics + CoreText,
                          and one image call, aspect-fit (#40).
*Exporter.swift           PDF generation, one file per document type (Stripboard, ShootingSchedule, the month
                          calendar (PDFExporter), CallSheet, Breakdown, DaysOutOfDays, ShotList). All draw on
                          PDFCanvas and build everywhere.
...
DerivedScheduleState.swift                                             What the editor derives from the whole project (sorted Boneyard,
                                                                       conflict sets, lock drift, the scene editor's suggestions and frame
                                                                       total) and the per-window cache that memoizes it.
...
PlatformPasteboardResponder, PlatformTabAccessory, PlatformListEditing, PlatformFrameSources,
PlatformEditorContainer, LegacyProjectHandoff, MacAppDelegate     Platform seams (ADR 0003).
...
3. `perform` registers the undo action with the window's `UndoManager`; that is what marks the
   ...
   On the Mac Edit ▸ Undo reaches that manager through NSDocument's window; on iOS and iPadOS
   the shake and the three-finger gestures reach it through the pasteboard responder, which
   carries it as UIKit's first responder (`PlatformPasteboardResponder`).
...
- A scene with shots has `estimatedTime` equal to the sum of their durations, and no frame of its
  own: every shot edit (`ShotEdits.swift`) ends with the estimate rule and the first shot takes the
  scene's frame. A shot never leaves its scene, and its letter is never stored.
- Frame bytes are only ever what `StoryboardFrame.encode` wrote (JPEG, at most 1200 px on the long
  edge); nothing stores an original image.

## Terms we avoid

- "Shot" for a scene; "block" for a strip; "task" or "event" for a scene. Use **scene** and **strip**;
  a **shot** is one setup inside a scene's shot list, never the scene itself.
