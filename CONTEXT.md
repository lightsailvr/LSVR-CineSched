...
| **Shoot day** | One calendar date in the production, modelled by `ShootDay`: a date, an ordered list of scenes, an optional call sheet, and a blackout flag. Every date in the production range has a `ShootDay`, even if empty. |
...
| **Blackout day** | A date marked unavailable (`ShootDay.isBlackout`). Scenes may still be placed there but are flagged. |
| **Shift mode** | When the start date moves, shift every scheduled scene by the same offset instead of leaving them on their absolute dates. Calendar events never shift. |
...
| **Calendar event** | A dated non-production item (travel day, rehearsal, scout). A `Scene` with `isCalendarEvent == true`. Never in the Boneyard, never counted as a scene, never shifted. |
...
- Calendar events keep their absolute date through range changes and shift mode.
