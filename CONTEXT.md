...
| **Storyboard frame** | One image on a shot, or on a scene with no shots, stored in the project file as downscaled JPEG bytes (`Shot.frame`, `Scene.frame`; base64 in the JSON, ADR 0007). The first shot added to a scene takes the scene's frame (the **frame rule**), so a scene with shots carries none of its own. |
