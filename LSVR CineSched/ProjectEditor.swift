...
// width (an iPhone, a narrow Split View or Slide Over pane) the minimal editor, which
// stands in until the iPhone companion of milestone 4 replaces it (story 54 of #1: the
// phone layout appears whenever a window is compact). The switch is the environment's
// horizontal size class, so a window resized across the boundary swaps editors; each
// keeps its own view state, and the document is the same object underneath.
//
