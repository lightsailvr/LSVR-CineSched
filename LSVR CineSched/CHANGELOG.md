...
- **A native `.cinesched` file type.** The app now declares `com.lsvr.cinesched.project` (kind "CineSched Project", extension `.cinesched`, a JSON file inside) so that the coming iPad, iPhone and Vision Pro document browser shows only schedules rather than every `.json` on the device. Nothing about saving changes yet: the Mac still writes `.json`, and a `.cinesched` renamed to `.json` opens in every older build because the contents are identical. Under the hood this build also carries the shared project document that every platform will edit through (reading, writing and a whole-project undo that includes call sheets, production info and the title); it is not wired to the Mac window yet.
