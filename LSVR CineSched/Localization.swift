...
// Global helper function for translations. A lookup in `localizationTable`, which is
// built once: this is called from nearly every view body (menu items, tooltips, labels),
// and building the table per call was the single largest cost of a calendar redraw (#34).
func L(_ key: String, lang: AppLanguage = LocalizationManager.shared.currentLanguage) -> String {
    localizationTable[key]?[lang] ?? key
}

/// Every user-facing string the app has a translation for, keyed by its English key.
let localizationTable: [String: [AppLanguage: String]] = [
    // Top Menus
